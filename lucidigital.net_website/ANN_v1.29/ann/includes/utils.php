...
if ( ! function_exists( 'ann_array_str_replace' ) ) {
	/**
	 ...
	 */
	function ann_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = ann_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'ann_get_svg_from_file' ) ) {
	...
	function ann_get_svg_from_file( $svg ) {
		if ( ann_is_url( $svg ) && ! ann_is_external_url( $svg ) ) {
			$svg_path = ann_url_to_local_path( $svg );
			if ( ! empty( $svg_path ) ) {
				$svg = $svg_path;
			}
		}
			...
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							...
							apply_filters( 'ann_filter_cookie_options', array(
								...
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//ann_get_domain_from_url( home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//ann_get_domain_from_url( home_url() ),
							$secure,
							$httponly
						);
			}
...
if ( ! function_exists( 'ann_rgba2hex' ) ) {
	/**
	 ...
	 */
	function ann_rgba2hex( $color, $alpha = false ) {
...
if ( ! function_exists( 'ann_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function ann_unserialize_recover( $str ) {
...
if ( ! function_exists( 'ann_is_external_url' ) ) {
	...
	function ann_is_external_url( $url ) {
		...
		return ann_is_url( $url ) && strpos( $url, ann_remove_protocol_from_url( $parts[0], true ) ) === false;
	}
}
