...
if ( ! function_exists( 'ann_wp_theme_setup' ) ) {
	...
	function ann_wp_theme_setup() {
		...
		// add_filter( 'ann_filter_privacy_text', 'ann_add_privacy_page_link' );
	}
}
...
if ( ! function_exists('ann_get_the_ID') ) {
	/**
	 * Return current post ID
	 * 
	 * @return int  Current post ID
	 */
	function ann_get_the_ID() {
		global $wp_query;
		return in_the_loop() 
					? get_the_ID() 
					: ( ! empty( $wp_query->post->ID )
						? $wp_query->post->ID
						: ( ann_is_singular() && ! empty( $wp_query->queried_object->ID )
...
if ( ! function_exists( 'ann_create_widgets_area' ) ) {
	...
	function ann_create_widgets_area( $name, $add_classes = '' ) {
		$widgets_name = ann_get_theme_option( $name, 'hide' );
...
if ( ! function_exists( 'ann_get_content_width' ) ) {
	...
	function ann_get_content_width() {
		$pg_width = apply_filters( 'ann_filter_content_width', (int)ann_get_theme_option( 'page_width' ) );
		$sb_width = 0;
		$sb_gap   = 0;
		if ( ann_sidebar_present() ) {
			...
			if ( 'default' == $ann_sidebar_type ) {
				...
				if ( is_active_sidebar( $ann_sidebar_name ) ) {
					$sb_width = (int)ann_get_theme_option( 'sidebar_width' );
					$sb_gap   = (int)ann_get_theme_option( 'sidebar_gap' );
				}
			} else {
				$sb_width = (int)ann_get_theme_option( 'sidebar_width' );
				$sb_gap   = (int)ann_get_theme_option( 'sidebar_gap' );
...
if ( ! function_exists( 'ann_get_date' ) ) {
	 ...
	 */
	function ann_get_date( $dt = '', $format = '', $disable_date_diff = false ) {
		$diff   = $disable_date_diff ? 0 : intval( ann_get_theme_option( 'time_diff_before' ) );
		...
		if ( $diff == 0 || $dt_cur - $dt > $diff * 24 * 3600 ) {
			...
			$dt = sprintf( esc_html__( '%s ago', 'ann' ), human_time_diff( $dt, $dt_cur ) );
		}
		return $dt;
	}
}
...
if ( ! function_exists( 'ann_get_privacy_text' ) ) {
	...
	function ann_get_privacy_text() {
		return apply_filters( 'ann_filter_privacy_text', wp_kses( trim( ann_get_theme_option( 'privacy_text' ) ), 'ann_kses_content' ) );
	}
}

if ( ! function_exists( 'ann_add_privacy_page_link' ) ) {
	/**
	 * Adds the Privacy Policy page link to the Privacy text
	 * 
	 * @hooked filter ann_filter_privacy_text
	 * 
	 * @param string $text - the text to modify
	 *
	 * @return string  A label text.
	 */
	function ann_add_privacy_page_link( $text ) {
		if ( ! empty( $text ) ) {
			...
			if ( ! empty( $page ) ) {
				$text .=  ( ! in_array( substr( $text, -1 ), array( '.', '!' ) ) ? '.' : '' )
							...
							. sprintf(
								__( 'For further details on handling user data, see our %s.', 'ann' ),
								'<a href="' . esc_url( get_permalink( $page ) ) . '" target="_blank">' . __( 'Privacy Policy', 'ann' ) . '</a>'
							);
			}
		}
		return $text;
...
if ( ! function_exists( 'ann_verify_nonce' ) ) {
	 ...
	 */
	function ann_verify_nonce( $nonce = 'nonce', $mask = '', $die = true ) {
		$rez = true;
		if ( empty( $mask ) ) {
...
if ( ! function_exists( 'ann_get_upgrade_url' ) ) {
	...
	function ann_get_upgrade_url( $params = array() ) {
		...
		if ( ! empty( $params['action'] ) && ! in_array( $params['action'], array( 'info_skins' ) ) ) {
			...
			$default['domain']     = ann_remove_protocol_from_url( home_url(), true );
