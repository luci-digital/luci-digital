...
if ( function_exists( 'ann_elementor_is_preview' ) && ann_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			ann_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}
	...
	ann_new_main_query(
		array(
			'post_type'      => ann_get_theme_option( 'post_type' ),
			'category'       => ann_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'ann_filter_get_template_part', ann_blog_archive_get_template() ) );
}
