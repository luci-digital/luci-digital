	...
	function ann_show_post_title_and_meta( $args = array() ) {
		...
		if ( ( ! ann_sc_layouts_showed( 'title' ) || ! ann_sc_layouts_showed( 'postmeta' ) ) ) {
			...
			<div class="post_header post_header_single entry-header">
				...
				$share_position  = ann_array_get_keys_by_value( ann_get_theme_option( 'share_position', 'top=0|left=0|bottom=1' ) );
	...
	function ann_show_post_footer( $components = 'pages,tags,likes,share,prev_next,author' ) {
		...
		foreach( $components as $comp ) {
			...
			if ( 'tags' == $comp ) {
				...
				// Emotions
				if ( ann_exists_trx_addons() && function_exists( 'trx_addons_get_post_reactions' ) && trx_addons_is_on( trx_addons_get_option( 'emotions_allowed', 0 ) ) ) {
				...
				if ( in_array( 'likes', $components ) && in_array( 'likes', $meta_components )
						&&
						( ! function_exists( 'trx_addons_get_option' ) || trx_addons_is_off( trx_addons_get_option( 'emotions_allowed', 0 ) ) || ! apply_filters( 'trx_addons_filter_show_post_reactions', ann_is_single() && ! is_attachment() ) )
				) {
					$meta_footer[] = 'likes';
				}
				...
				if ( ann_get_theme_option( 'show_author_info' ) == 1
					&& ! is_attachment()
					&& ( apply_filters( 'ann_filter_show_author_without_bio', true ) || get_the_author_meta( 'description' ) )
					&& ( 'scroll' != $ann_posts_navigation || (int)ann_get_theme_option( 'posts_navigation_scroll_hide_author', 0 ) == 0 )
					&& ( ! $full_post_loading || (int)ann_get_theme_option( 'open_full_post_hide_author', 1 ) == 0 )
				) {
	...
	function ann_show_post_meta( $args = array() ) {
		...
		foreach ( $components as $comp ) {
			...
			if ( 'categories' == $comp ) {
				...
				// Published date
				$dt = apply_filters( 'ann_filter_get_post_date', ann_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '', ! empty( $args['disable_date_diff'] ) ) );
				...
				// Modified date
				$dt = apply_filters( 'ann_filter_get_post_modified_date', ann_get_date( get_post_modified_time( 'U' ), ! empty( $args['date_format'] ) ? $args['date_format'] : '', ! empty( $args['disable_date_diff'] ) ) );
				if ( ! empty( $dt ) && ( empty( $dt_last ) || $dt_last != $dt ) ) {
					ann_show_layout(
						$dt,
				...
				if ( !ann_is_single() || have_comments() || comments_open() ) {
					...
					echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-light">'
							...
							. ( $args['show_labels'] ? '<span class="post_meta_label">' . esc_html( _nx( 'Comment', 'Comments', $post_comments, 'Number of comments', 'ann' ) ) . '</span>' : '' )
						. '</a>';
				}
				...
				if ( function_exists( 'trx_addons_get_post_likes' ) ) {
					...
					echo ( true == $emotions_allowed
							...
							: '<a href="#"'
								...
								. ' title="' . ( ! empty( $allow ) ? esc_attr( _x( 'Like', 'Link title', 'ann' ) ) : esc_attr( _x( 'Dislike', 'Link title', 'ann' ) ) ) . '"'
								. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
								. ' data-likes="' . esc_attr( $post_likes ) . '"'
								. ' data-title-like="' . esc_attr( _x( 'Like', 'Link title', 'ann' ) ) . '"'
								. ' data-title-dislike="' . esc_attr( _x( 'Dislike', 'Link title', 'ann' ) ) . '"'
								. '>'
							)
								...
								. ( $args['show_labels']
									? '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? esc_html( _nx( 'Reaction', 'Reactions', $post_likes, 'Number of reactions', 'ann' ) )
											: esc_html( _nx( 'Like', 'Likes', $post_likes, 'Number of likes', 'ann' ) )
											)
									. '</span>'
									: '' )
							. '</a>';
...
if ( ! function_exists( 'ann_show_post_featured' ) ) {
	...
	function ann_show_post_featured( $args = array() ) {
		$args = apply_filters( 'ann_filter_post_featured_args', array_merge(
			array(
				'popup'         => ! ann_is_single() && (int)ann_get_theme_option( 'video_in_popup', 0 ) > 0,  // Open video in popup
				'hover'         => ann_get_theme_option( 'image_hover' ),     // Hover effect
				'parallax'      => (int)ann_get_theme_option( 'single_parallax', 0 ), // Parallax speed. If 0 - no parallax effect is used
		...
		if ( ! empty( $args['singular'] ) ) {
			...
			if ( is_attachment() ) {
				...
				if ( ( ( $has_thumb || ! empty( $args['show_no_image'] ) ) && ( ! ann_sc_layouts_showed( 'featured' ) || ! empty( $args['force'] ) ) )
		...
		if ( ! $args['thumb_only'] && ( in_array( $args['post_format'], array( 'video', 'audio', 'gallery' ) ) || ! empty( $args['video'] ) ) ) {
			...
			if ( 'video' == $args['post_format'] || ! empty( $args['video'] ) ) {
				if ( ! empty( $args['singular'] ) 
					...
					} else if ( ann_get_theme_option( 'blog_content', 'excerpt' ) != 'fullpost' ) {
				...
				if ( empty( $args['singular'] ) ) {
					if ( empty( $audio ) && ann_get_theme_option( 'blog_content', 'excerpt' ) != 'fullpost' ) {
...
if ( ! function_exists( 'ann_get_no_image' ) ) {
	 ...
	 */
	function ann_get_no_image( $no_image = '', $need = false, $return_url = true ) {
		...
		if ( empty( $img ) && ( $need || ann_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( $return_url ) {
				...
				if ( empty( $no_image_dir ) ) {
					$no_image_dir = ann_get_file_dir( 'images/no-image.jpg' );
				}
				$img = $no_image_dir;
			}
		}
...
if ( ! function_exists( 'ann_show_related_posts_callback' ) ) {
	...
	function ann_show_related_posts_callback() {
		if ( ann_is_single() && ! apply_filters( 'ann_filter_show_related_posts', false ) ) {
			...
			$ann_related_style   = ann_get_theme_option( 'related_style', 'classic' );
...
if ( ! function_exists( 'ann_show_related_posts' ) ) {
	...
	function ann_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int)ann_get_theme_option( 'related_slider', 0 ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = ann_get_theme_option( 'related_position', 'below_content' );
			$related_style = ann_get_theme_option( 'related_style', 'classic' );
			...
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				...
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = (int)ann_get_theme_option( 'related_slider_space', 30 );
					$slider_args['slider_controls']   = ann_get_theme_option( 'related_slider_controls', 'none' );
					$slider_args['slider_pagination'] = ann_get_theme_option( 'related_slider_pagination', 'bottom' );
...
if ( ! function_exists( 'ann_show_posts' ) ) {
	...
	function ann_show_posts( $args = array() ) {
		...
		$class = apply_filters( 'ann_filter_posts_container_classes', 'posts_container'
				...
				. ( ! ann_is_off( $custom_style )
					...
					: ( ann_is_blog_style_use_masonry( $style )
						...
						: ( $columns > 1 || apply_filters( 'ann_filter_columns_wrap_for_single_column', false )
		...
		if ( $args['sticky'] ) {
			...
			if ( (int)ann_get_theme_option( 'first_post_large', 0 ) > 0 && ! is_paged() && ! in_array( ann_get_theme_option( 'body_style' ), array( 'fullwide', 'fullscreen' ) ) && have_posts() ) {
				the_post();
