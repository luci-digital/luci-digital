<div class="front_page_section front_page_section_blog<?php
	$ann_scheme = ann_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $ann_scheme ) && ! ann_is_inherit( $ann_scheme ) ) {
		echo ' scheme_' . esc_attr( $ann_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( ann_get_theme_option( 'front_page_blog_paddings' ) );
	if ( ann_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$ann_css      = '';
		$ann_bg_image = ann_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$ann_anchor_icon = ann_get_theme_option( 'front_page_blog_anchor_icon' );
	$ann_anchor_text = ann_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( ann_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$ann_css      = '';
			$ann_bg_mask  = ann_get_theme_option( 'front_page_blog_bg_mask' );
			$ann_bg_color_type = ann_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $ann_bg_color_type ) {
				$ann_bg_color = ann_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$ann_caption = ann_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $ann_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $ann_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $ann_caption, 'ann_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$ann_description = ann_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $ann_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $ann_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $ann_description ), 'ann_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! ann_exists_trx_addons() ) {
						...
						ann_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
