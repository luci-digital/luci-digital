...
if ( ! function_exists( 'ann_theme_setup9' ) ) {
	...
	function ann_theme_setup9() {
		...
		$GLOBALS['content_width'] = apply_filters( 'ann_filter_content_width', (int)ann_get_theme_option( 'page_width' ) );
		...
		register_nav_menus( apply_filters( 'ann_filter_register_nav_menus', array(
		...
		if ( $mult > 1 ) {
			$GLOBALS['content_width'] = apply_filters( 'ann_filter_content_width', (int)ann_get_theme_option( 'page_width' ) * $mult );
		}
...
if ( ! function_exists( 'ann_wp_styles_single' ) ) {
	...
	function ann_wp_styles_single() {
		if ( apply_filters( 'ann_filters_separate_single_styles', false )
			&& apply_filters( 'ann_filters_load_single_styles',
								ann_is_single()
								|| ann_is_singular( 'attachment' )
								|| (int)ann_get_theme_option( 'open_full_post_in_blog', 0 ) > 0
								|| ( ann_is_singular( 'page' ) && ! is_front_page() && ( comments_open() || get_comments_number() ) )
							)
		) {
...
if ( ! function_exists( 'ann_wp_styles_single_responsive' ) ) {
	...
	function ann_wp_styles_single_responsive() {
		if ( apply_filters( 'ann_filters_separate_single_styles', false )
			&& apply_filters( 'ann_filters_load_single_styles', ann_is_single() || ann_is_singular( 'attachment' ) || (int)ann_get_theme_option( 'open_full_post_in_blog', 0 ) > 0 )
		) {
			if ( ann_is_off( ann_get_theme_option( 'debug_mode' ) ) ) {
...
if ( ! function_exists( 'ann_wp_scripts' ) ) {
	...
	function ann_wp_scripts() {
		...
		if ( (int)ann_get_theme_option( 'single_parallax', 0 ) != 0
			&& ( ann_is_singular( 'post' )
				|| ( (int)ann_get_theme_option( 'open_full_post_in_blog', 0 ) > 0 && ( is_home() || is_archive() || is_category() ) )
				)
		) {
		...
		if ( $blog_archive && ! is_customize_preview() && ! ann_is_off( ann_get_theme_option( 'show_filters', 0 ) ) ) {
...
if ( ! function_exists( 'ann_localize_scripts' ) ) {
	...
	function ann_localize_scripts() {
		...
		wp_localize_script( 'ann-init', 'ANN_STORAGE', apply_filters( 'ann_filter_localize_script', array(
			...
			// Sidemenu options
			'menu_side_stretch'   => (int) ann_get_theme_option( 'menu_side_stretch', 0 ) > 0,
			'menu_side_icons'     => (int) ann_get_theme_option( 'menu_side_icons', 0 ) > 0,
			...
			'open_full_post'      => (int)ann_get_theme_option( 'open_full_post_in_blog', 0 ) > 0,

			// Which block to load in the single posts
			'which_block_load'    => ann_get_theme_option( 'posts_navigation_scroll_which_block', 'article' ),
...
if ( ! function_exists( 'ann_add_body_classes' ) ) {
	...
	function ann_add_body_classes( $classes ) {
		...
		if ( ann_sidebar_present() ) {
			...
			$classes[] = 'sidebar_small_screen_' . esc_attr( ann_get_theme_option( 'sidebar_position_ss', 'below' ) );
		} else {
		...
		if ( ann_get_theme_option( 'blog_content', 'excerpt' ) == 'fullpost' ) {
...
if ( ! function_exists( 'ann_wp_footer' ) ) {
	...
	function ann_wp_footer() {
		// Add header zoom
		$header_zoom = max( 0.2, min( 2, (float)ann_get_theme_option( 'header_zoom', 1 ) ) );
		if ( 1 != $header_zoom ) {
			ann_add_inline_css( ".sc_layouts_title_title{font-size:{$header_zoom}em}" );
		}
		// Add logo zoom
		$logo_zoom = max( 0.2, min( 2, (float)ann_get_theme_option( 'logo_zoom', 1 ) ) );
		if ( 1 != $logo_zoom ) {
		...
		if ( ! empty( $css ) ) {
			...
			ann_show_layout( apply_filters( 'ann_filter_inline_css', $css ), '<style type="text/css" id="ann-inline-styles-inline-css">', '</style>' );
		}
	}
}
...
require_once ann_get_file_dir( 'theme-specific/theme-improves.php' );
