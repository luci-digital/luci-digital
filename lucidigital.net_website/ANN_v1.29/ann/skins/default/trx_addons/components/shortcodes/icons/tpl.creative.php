...
$icon_present = '';
$svg_present = false;
$svg_bg_dir = trx_addons_get_svg_from_file(ann_get_file_dir('images/decor.svg'));
        ...
        ?><div class="sc_icons_item<?php echo esc_attr(( !empty($item['link']) ) ? ' sc_icons_item_linked' : '');
			...
			if (!empty($item['char'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_icons_image"<?php trx_addons_sc_show_attributes('sc_icons', $args, 'sc_item_wrapper'); ?>><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'ann'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
                        if ( !empty ( $svg_bg_dir ) ) {?>
                            <div class="sc_icons_decoration_bg"><?php trx_addons_show_layout( $svg_bg_dir );?></div><?php
                        }
                    ?></div><?php
